...
app.post('/api/upload/template', upload.single('file'), (req, res) => {
  ...
  const targetPath = path.join(templatesDir, originalName);
...
let activeGeneration: { aborted: boolean } | null = null;
...
app.post('/api/templates/:name/rename', (req, res) => {
  try {
    ...
    const newPath = path.join(templatesDir, newName.trim());
...
app.get('/api/download', (req, res) => {
  ...
  if (!resolved.startsWith(outputDir) && !resolved.startsWith(templatesDir)) {
